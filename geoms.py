...
def filter_mesh_by_cylinder2(mesh, cylinder_radius, cylinder_height, cylinder_center=None):
    """
    Filters the mesh to keep only the vertices inside a bounding cylinder.
    ...
    # Compute mask for vertices inside the cylinder
    translated_vertices = vertices - cylinder_center  # Shift cylinder center to origin
    x, y, z = translated_vertices[:, 0], translated_vertices[:, 1], translated_vertices[:, 2]
    
    inside_cylinder_mask = (np.sqrt(x**2 + y**2) <= cylinder_radius) & (np.abs(z) <= cylinder_height / 2)
    
    # Get indices of the vertices inside the cylinder (convert to list)
    indices_to_keep = np.where(inside_cylinder_mask)[0].tolist()

    # Filter the mesh using the selected indices
    filtered_mesh = mesh.select_by_index(indices_to_keep)  # No retain_triangle argument
